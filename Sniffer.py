...
def ethernetWindowRow2Cols(root: Toplevel, l: str, r: str, **args):
...
def ethernetWindow(root: Tk, e_packet: EPacket):
    ...
    ethernetWindowRow2Cols(frame, 'MAC destino', hexToMAC(e_frame.getField(*EFrame.DESTINATION_ADDRESS)))
    ethernetWindowRow2Cols(frame, 'MAC origen', hexToMAC(e_frame.getField(*EFrame.SOURCE_ADDRESS)))
    ethernetWindowRow2Cols(frame, 'Tipo de servicio', SERVICES.get(e_frame.getField(*EFrame.TYPE), 'Undefined'))
    ...
    ethernetWindowRow2Cols(frame, 'Version', ip_dp.getField(*IPDataPacket.VERSION))
    ethernetWindowRow2Cols(frame, 'Longitud (bytes)', int(ip_dp.getField(*IPDataPacket.VERSION)) * int(ip_dp.getField(*IPDataPacket.IHL)))
    
    # TOS - Type of service
    tos = ip_dp.getField(*IPDataPacket.TOS)
    ethernetWindowRow2Cols(frame, 'TOS', tos.hexData())
    ethernetWindowRow2Cols(frame, '', 
    ...
    ethernetWindowRow2Cols(frame, 'Longitud total del paquete', ip_dp.getField(*IPDataPacket.TOTAL_LENGTH))
    ethernetWindowRow2Cols(frame, '', str(int(ip_dp.getField(*IPDataPacket.TOTAL_LENGTH), HEX_SCALE)) + ' bytes',fg='green')
    ethernetWindowRow2Cols(frame, 'Identificacion', ip_dp.getField(*IPDataPacket.IDENTIFICATION))
    ethernetWindowRow2Cols(frame, 'Banderas', ip_dp.getField(*IPDataPacket.FLAGS))
    ethernetWindowRow2Cols(frame, '',
        ...
        fg='green', height=3)
    ethernetWindowRow2Cols(frame, 'Desplazamiento de fragmentacion', ip_dp.getField(*IPDataPacket.OFFSET))
    ethernetWindowRow2Cols(frame, 'Tiempo de vida', ip_dp.getField(*IPDataPacket.TTL))
    ethernetWindowRow2Cols(frame, '', str(int(ip_dp.getField(*IPDataPacket.TTL), HEX_SCALE)) + ' segundos',fg='green')
    ethernetWindowRow2Cols(frame, 'Protocolo', ip_dp.getField(*IPDataPacket.PROTOCOL))
    ethernetWindowRow2Cols(frame, '', IP_PROTOCOL.get(ip_dp.getField(*IPDataPacket.PROTOCOL), 'Undefined'),fg='green')
    ethernetWindowRow2Cols(frame, 'Checksum', ip_dp.getField(*IPDataPacket.CHECKSUM))
    checksum_correct = True if ip_dp.verifyChecksum() == "0xffff" else False
    ethernetWindowRow2Cols(frame, '', 'Checksum result: ' + ('OK' if checksum_correct else 'INCORRECT'),fg='green' if checksum_correct else 'red')
    ethernetWindowRow2Cols(frame, 'IP Origen', hexToIP(ip_dp.getField(*IPDataPacket.SOURCE_ADDRESS)))
    ethernetWindowRow2Cols(frame, 'IP Destino', hexToIP(ip_dp.getField(*IPDataPacket.DESTINATION_ADDRESSS)))
    ...
    ethernetWindowRow2Cols(frame, 'Direccion puerto origen', int(tcp.getField(*TCP.SOURCE_PORT), HEX_SCALE))
    ethernetWindowRow2Cols(frame, 'Direccion puerto destino', int(tcp.getField(*TCP.DESTINATION_PORT), HEX_SCALE))
    ethernetWindowRow2Cols(frame, 'Numero de secuencia', tcp.getField(*TCP.SEQUENCE_NUMBER))
    ethernetWindowRow2Cols(frame, 'Numero de confirmacion', tcp.getField(*TCP.ACK))
    ethernetWindowRow2Cols(frame, 'Longitud de cabecera TCP', tcp.getField(*TCP.HEADER_LENGTH))
    ethernetWindowRow2Cols(frame, '', str(int(tcp.getField(*TCP.HEADER_LENGTH), BIN_SCALE) * int(ip_dp.getField(*IPDataPacket.VERSION))) + ' bytes',fg='green')
    ethernetWindowRow2Cols(frame, 'Reservado', tcp.getField(*TCP.RESERVED))
    ethernetWindowRow2Cols(frame, 'Flags', tcp.getField(*TCP.FLAGS))
    tcp_flags = tcp.getField(*TCP.FLAGS)
    ethernetWindowRow2Cols(frame, 'Flags', 
        'Nonce: ' + tcp_flags[0] + 
        ...
        '\nFin: ' + tcp_flags[8],
        fg='green', height=9)
    ethernetWindowRow2Cols(frame, 'Tamano de la ventana', int(tcp.getField(*TCP.WINDOW_SZ), HEX_SCALE))
    ethernetWindowRow2Cols(frame, 'Checksum', tcp.getField(*TCP.CHECKSUM))
    ethernetWindowRow2Cols(frame, 'Punto Urgente', tcp.getField(*TCP.URGENT_POINT))
